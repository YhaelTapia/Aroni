...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['premio_id'])) {
    ...
    if ($saldo >= $p['costo']) {
        ...
        $conn->query("UPDATE usuarios SET meentcoins = meentcoins - {$p['costo']} WHERE id = $usuario_id");
        $conn->query("INSERT INTO canjes (usuario_id, premio_id) VALUES ($usuario_id, $premio_id)");

        $conn->commit();
        $mensaje = "Canje exitoso: " . $p['nombre'];
...
<head>
    ...
    <link rel="stylesheet" href="css/tienda.css?v=9.0"> <!-- Cache busting -->
</head>
<body>
    <nav class="top-nav">
